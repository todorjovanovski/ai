...
if __name__ == "__main__":
    ...
    train_x = list()
    for t in train_set:
        row = [t[i] for i in range(len(t)-1) if i != col]
        train_x.append(row)
    ...
    classifier = RandomForestClassifier(criterion=criteria, random_state=0, n_estimators=num_trees)
    classifier.fit(train_x, train_y)

    accuracy = 0
    for i in range(len(test_set)):
        guess = classifier.predict([test_x[i]])[0]
        actual = test_y[i]
        if actual == guess:
            accuracy += 1
    ...
    predicted = classifier.predict([test])[0]
