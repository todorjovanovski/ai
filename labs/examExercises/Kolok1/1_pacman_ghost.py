...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        return 0 <= state[0] < n and 0 <= state[1] < n and state not in walls
    ...
    def successor(self, state):
        ...
        # up 1
        new_pos = (x, y + 1)
        if self.check_valid(new_pos, self.walls, self.n):
            successors["Gore 1"] = new_pos
        # up 2
        new_pos = (x, y + 2)
        if self.check_valid(new_pos, self.walls, self.n):
            successors["Gore 2"] = new_pos
        # up 3
        new_pos = (x, y + 3)
        if self.check_valid(new_pos, self.walls, self.n):
            successors["Gore 3"] = new_pos

        # right 1
        new_pos = (x + 1, y)
        if self.check_valid(new_pos, self.walls, self.n):
            successors["Desno 1"] = new_pos
        # right 2
        new_pos = (x + 2, y)
        if self.check_valid(new_pos, self.walls, self.n):
