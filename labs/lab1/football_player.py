...
class Football(Problem):
    ...
    def __init__(self, opponents_pos, blocked, initial, goal=None):
    ...
    @staticmethod
    def check_valid(state, illegal, opponents):
        man_pos = state[0]
        ball_pos = state[1]
        return man_pos[0] >= 0 and man_pos[0] < 8 and man_pos[1] >= 0 and man_pos[1] < 6 and ball_pos[0] >= 0 and \
            ball_pos[0] < 8 and ball_pos[1] >= 0 and ball_pos[1] < 6 and man_pos not in opponents and ball_pos not in illegal
    ...
    def successor(self, state):
        ...
        # gore
        if self.check_valid(((man_x, man_y + 1), (ball_x, ball_y)), self.blocked, self.opponents_pos):
            if (man_x, man_y + 1) == (ball_x, ball_y):
                if self.check_valid(((man_x, man_y + 1), (ball_x, ball_y + 1)), self.blocked, self.opponents_pos):
                    successors["Turni topka gore"] = ((man_x, man_y + 1), (ball_x, ball_y + 1))
            else:
                successors["Pomesti coveche gore"] = ((man_x, man_y + 1), (ball_x, ball_y))

        # dolu
        if self.check_valid(((man_x, man_y - 1), (ball_x,  ball_y)), self.blocked, self.opponents_pos):
            if (man_x, man_y - 1) == (ball_x, ball_y):
                if self.check_valid(((man_x, man_y - 1), (ball_x, ball_y - 1)), self.blocked, self.opponents_pos):
                    successors["Turni topka dolu"] = ((man_x, man_y - 1), (ball_x, ball_y - 1))
            else:
                successors["Pomesti coveche dolu"] = ((man_x, man_y - 1), (ball_x, ball_y))

        # desno
        if self.check_valid(((man_x + 1, man_y), (ball_x,  ball_y)), self.blocked, self.opponents_pos):
            if (man_x + 1, man_y) == (ball_x, ball_y):
                if self.check_valid(((man_x + 1, man_y), (ball_x + 1, ball_y)), self.blocked, self.opponents_pos):
                    successors["Turni topka desno"] = ((man_x + 1, man_y), (ball_x + 1, ball_y))
            else:
                successors["Pomesti coveche desno"] = ((man_x + 1, man_y), (ball_x, ball_y))

        # gore desno
        if self.check_valid(((man_x + 1, man_y + 1), (ball_x, ball_y)), self.blocked, self.opponents_pos):
            if (man_x + 1, man_y + 1) == (ball_x, ball_y):
                if self.check_valid(((man_x + 1, man_y + 1), (ball_x + 1, ball_y + 1)), self.blocked, self.opponents_pos):
                    successors["Turni topka gore-desno"] = ((man_x + 1, man_y + 1), (ball_x + 1, ball_y + 1))
            else:
                successors["Pomesti coveche gore-desno"] = ((man_x + 1, man_y + 1), (ball_x, ball_y))

        # dolu-desno
        if self.check_valid(((man_x + 1, man_y - 1), (ball_x, ball_y)), self.blocked, self.opponents_pos):
            if (man_x + 1, man_y - 1) == (ball_x, ball_y):
                if self.check_valid(((man_x + 1, man_y - 1), (ball_x + 1, ball_y - 1)), self.blocked, self.opponents_pos):
                    successors["Turni topka dolu-desno"] = ((man_x + 1, man_y - 1), (ball_x + 1, ball_y - 1))
            else:
                successors["Pomesti coveche dolu-desno"] = ((man_x + 1, man_y - 1), (ball_x, ball_y))
...
if __name__ == '__main__':
    ...
    opponents = [(3, 3), (5, 4)]
    illegal = [(2,2), (3,2), (4,2), (2,3), (3,3), (4,3), (2,4), (3,4), (4,4), (4,3), (5,3), (6,3), (4,4), (5,4), (6,4), (4,5), (5,5), (6,5)]

    football = Football(opponents, illegal, (man_pos, ball_pos), ((7, 2), (7, 3)))
