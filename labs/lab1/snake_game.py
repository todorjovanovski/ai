...
class Snake(Problem):
    ...
    def check_validity(self, state, body):
        head_x, head_y = state[0], state[1]
        if 0 <= head_x < 10 and 0 <= head_y < 10 and state not in body and state not in tuple(self.red_apples):
            return True

    def successor(self, state):
        successors = dict()

        snake_blocks = state[0]
        snake_direction = state[1]
        green_apples = state[2]
        snake_head = snake_blocks[0]
        head_x = snake_head[0]
        head_y = snake_head[1]

        # when facing down
        if snake_direction == "downward":
            # keep straight
            if self.check_validity((head_x, head_y - 1), snake_blocks):
                new_head = (head_x, head_y-1)
                new_blocks = [new_head]
                for block in snake_blocks[:-1]:
                    new_blocks.append(block)
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["ProdolzhiPravo"] = (tuple(new_blocks), "downward", tuple(apple for apple in green_apples if apple != new_head))

            # turn right
            if self.check_validity((head_x - 1, head_y), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiDesno"] = (tuple(new_blocks), "leftward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x + 1, head_y), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiLevo"] = (tuple(new_blocks), "rightward", tuple(apple for apple in green_apples if apple != new_head))

        # when facing up
        if snake_direction == "upward":
            # keep straight
            if self.check_validity((head_x, head_y + 1), snake_blocks):
                new_head = (head_x, head_y + 1)
                new_blocks = [new_head]
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["ProdolzhiPravo"] = (tuple(new_blocks), "upward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x + 1, head_y), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiDesno"] = (tuple(new_blocks), "rightward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x - 1, head_y), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiLevo"] = (tuple(new_blocks), "leftward", tuple(apple for apple in green_apples if apple != new_head))

        # when facing left
        if snake_direction == "leftward":
            # keep straight
            if self.check_validity((head_x - 1, head_y), snake_blocks):
                new_head = (head_x - 1, head_y)
                new_blocks = [new_head]
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["ProdolzhiPravo"] = (tuple(new_blocks), "leftward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x, head_y + 1), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiDesno"] = (tuple(new_blocks), "upward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x, head_y - 1), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiLevo"] = (tuple(new_blocks), "downward", tuple(apple for apple in green_apples if apple != new_head))

        # when facing right
        if snake_direction == "rightward":
            # keep straight
            if self.check_validity((head_x + 1, head_y), snake_blocks):
                new_head = (head_x + 1, head_y)
                new_blocks = [new_head]
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["ProdolzhiPravo"] = (tuple(new_blocks), "rightward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x, head_y - 1), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiDesno"] = (tuple(new_blocks), "downward", tuple(apple for apple in green_apples if apple != new_head))
            ...
            if self.check_validity((head_x, head_y + 1), snake_blocks):
                ...
                if new_head in green_apples:
                    new_blocks.append(snake_blocks[-1])
                successors["SvrtiLevo"] = (tuple(new_blocks), "upward", tuple(apple for apple in green_apples if apple != new_head))
...
if __name__ == '__main__':
    ...
    num_of_green = int(input())
    greens = []
    for i in range(num_of_green):
        temp = input().split(",")
        greens.append((int(temp[0]), int(temp[1])))

    num_of_red = int(input())
    reds = []
    for i in range(num_of_red):
        temp = input().split(",")
        reds.append((int(temp[0]), int(temp[1])))

    snake = Snake(reds, (snake_position, "downward", tuple(greens)))
